...
bool JitWarmUp::commit_compilation(methodHandle m, int bci, TRAPS) {
  ...
  if (CompilationPolicy::can_be_compiled(m, comp_level)) {
    ...
    CompileBroker::compile_method(m, bci, comp_level,
                                  methodHandle(), 0,
                                  CompileTask::Reason_JWarmUp, THREAD);
    return true;
  }
  return false;
}
...
int ProfileRecorder::compute_crc32(randomAccessFileStream* fs) {
  long old_postion = fs->ftell();
  fs->fseek(HEADER_SIZE, SEEK_SET);
  long content_size = fs->fileSize() - HEADER_SIZE;
  assert(content_size > 0, "sanity check");
  int loops = (int)(content_size / CRC32_BUF_SIZE);
  int rest_size = (int)(content_size % CRC32_BUF_SIZE);
  int crc = 0;
...
void ProfileRecorder::write_header() {
  ...
  *(unsigned int*)((char*)header_buf + offset) = recorded_count();
  ...
  // record time
  *(unsigned long*)((char*)header_buf + offset) = os::javaTimeMillis();
  _pos += TIME_WIDTH;
  offset += TIME_WIDTH;
  // write to file
...
bool PreloadMethodHolder::is_alive() const {
...
void PreloadClassChain::record_loaded_class(InstanceKlass* k) {
  ...
  if (holder != NULL) {
    if (holder->resolved()) {
      ...
      MutexLocker mu(PreloadClassChain_lock);
  ...
  //set entry to loaded
  {
    ...
    if (chain_entry->is_skipped()) {
      ...
      log_warning(warmup)("[JitWarmUp] WARNING : loaded class %s at index %d is ignored",
                          k->name()->as_C_string(), chain_index);
      return;
    } else if (chain_entry->is_initialized()) {
  ...
  for ( int index = 0; index < length(); index++ ) {
    ...
    Stack<PreloadMethodHolder*, mtInternal> compile_queue;
    {
      ...
      switch(entry->state()) {
        ...
        case PreloadClassChainEntry::_is_initialized:
    ...
    // initialize loaded and uninitialized klass
    if (klass != NULL) {
...
void PreloadClassChain::refresh_indexes() {
  ...
  for (int i = inited + 1; i < length(); i++) {
    ...
    if (e->is_loaded() && i == loaded + 1) {
      loaded = i;
    } else if (e->is_initialized() && i == inited + 1) {
      loaded = i;
      inited = i;
...
void PreloadClassChain::do_unloading() {
  assert(SafepointSynchronize::is_at_safepoint(), "must be in safepoint");
  if (deopt_has_done()) {
    return;
  }
...
#define PERLOAD_CLASS_HS_SIZE   10240
...
void PreloadJitInfo::init() {
  ...
  _dict = new PreloadClassDictionary(PERLOAD_CLASS_HS_SIZE);
