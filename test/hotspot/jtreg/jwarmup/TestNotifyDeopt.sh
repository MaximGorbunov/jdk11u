...
if [ "X$OS" != "XLinux" ]
then
    echo "JWarmUp only supported on Linux"
    exit 0;
fi
...
#run Recording Model
${JAVA} -XX:-TieredCompilation -XX:-UseSharedSpaces -XX:+CompilationWarmUpRecording -XX:-ClassUnloading -Xlog:warmup=debug -XX:CompilationWarmUpRecordTime=10 -XX:CompilationWarmUpLogfile=./jitwarmup.log -cp ${TESTCLASSES} ${TEST_CLASS} recording > output.txt  2>&1
sleep 1
${JAVA} -XX:-TieredCompilation -XX:-UseSharedSpaces -XX:+CompilationWarmUp -Xlog:warmup=debug -XX:CompileThreshold=500000 -XX:CompilationWarmUpLogfile=./jitwarmup.log -XX:+CompilationWarmUpExplicitDeopt -XX:CompilationWarmUpDeoptTime=1200 -cp ${TESTCLASSES} ${TEST_CLASS} compilation > output.txt  2>&1
...
function check_output()
{
  ...
  warmup_methods=`grep "preload method" output.txt|awk '{print $6}'`
  deopt_methods=`grep "WARNING : deoptimize warmup method" output.txt|awk '{print $8}'`
