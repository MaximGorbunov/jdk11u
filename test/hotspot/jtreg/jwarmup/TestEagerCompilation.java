...
public class TestEagerCompilation {
    ...
    public static void main(String[] args) throws Exception {
        ...
        /*
        output = testJitWarmDeopt(fileName);
        String[] ss = output.toString().split("\n");
        Pattern p = Pattern.compile("preload method (.+?)\\(");
        Pattern p2 = Pattern.compile("\\d+ (.+?) \\((.+?) made not entrant");
        Set<String> setSuccess = new HashSet<String>();
        Set<String> setNotEntrant = new HashSet<String>();
        for (String s : ss) {
            Matcher m = p.matcher(s);
            if (m.find()) {
                System.out.println(m.groupCount() + m.group(1));
                setSuccess.add(m.group(1));
            }
            Matcher m2 = p2.matcher(s);
            if (m2.find()) {
                String s2 = m2.group(1);
                String[] sl = s2.split(" ");
                s2 = sl[sl.length - 1].replace("::", ".");
                System.out.println(m2.groupCount() + s2);
                setNotEntrant.add(s2);
            }
        }
        for (String s : setSuccess) {
            if (!setNotEntrant.contains(s)) {
    ...
    public static class InnerA {
        private static JWarmUp jwarmup = JWarmUp.getJWarmUp();
        static {
